...
/* This example accompanies the book
 ...
 Copyright 2014 by Jonathan W. Valvano, valvano@mail.utexas.edu
    ...
    as int32_t as the above copyright notice remains
...
void static lcdwrite(enum typeOfWrite type, int8_t message){
  if(type == COMMAND){
...
//********Nokia5110_Outint8_t*****************
// Print a int8_tacter to the Nokia 5110 48x84 LCD.  The
// int8_tacter will be printed at the current cursor position,
// the cursor will automatically be updated, and it will
// wrap to the next row or back to the top if necessary.
// One blank column of pixels will be printed on either side
// of the int8_tacter for readability.  Since int8_tacters are 8
// pixels tall and 5 pixels wide, 12 int8_tacters fit per row,
// and there are six rows.
// inputs: data  int8_tacter to print
// outputs: none
// assumes: LCD is in default horizontal addressing mode (V = 0)
void Nokia5110_Outint8_t(uint8_t data){
  int i;
...
//********Nokia5110_OutString*****************
// Print a string of int8_tacters to the Nokia 5110 48x84 LCD.
...
void Nokia5110_OutString(int8_t *ptr){
  while(*ptr){
    Nokia5110_Outint8_t((uint8_t)*ptr);
    ptr = ptr + 1;
  }
}
...
void Nokia5110_OutUDec(uint16_t n){
  if(n < 10){
    Nokia5110_OutString("    ");
    Nokia5110_Outint8_t(n+'0'); /* n is between 0 and 9 */
  } else if(n<100){
    Nokia5110_OutString("   ");
    Nokia5110_Outint8_t(n/10+'0'); /* tens digit */
    Nokia5110_Outint8_t(n%10+'0'); /* ones digit */
  } else if(n<1000){
    Nokia5110_OutString("  ");
    Nokia5110_Outint8_t(n/100+'0'); /* hundreds digit */
    n = n%100;
    Nokia5110_Outint8_t(n/10+'0'); /* tens digit */
    Nokia5110_Outint8_t(n%10+'0'); /* ones digit */
  }
    ...
    Nokia5110_Outint8_t(' ');
    Nokia5110_Outint8_t(n/1000+'0'); /* thousands digit */
    n = n%1000;
    Nokia5110_Outint8_t(n/100+'0'); /* hundreds digit */
    n = n%100;
    Nokia5110_Outint8_t(n/10+'0'); /* tens digit */
    ...
    Nokia5110_Outint8_t(n/10000+'0'); /* ten-thousands digit */
    n = n%10000;
    Nokia5110_Outint8_t(n/1000+'0'); /* thousands digit */
    n = n%1000;
    Nokia5110_Outint8_t(n/100+'0'); /* hundreds digit */
...
// next int8_tacter will be printed here.  X=0 is the leftmost
...
void Nokia5110_SetCursor(uint8_t newX, uint8_t newY){
  ...
  // multiply newX by 7 because each int8_tacter is 7 columns wide
...
// outputs: none
// assumes: LCD is in default horizontal addressing mode (V = 0)
void Nokia5110_DrawFullImage(const int8_t *ptr){
  int i;
  Nokia5110_SetCursor(0, 0);
  for(i=0; i<(MAX_X*MAX_Y/8); i=i+1){
    lcdwrite(DATA, ptr[i]);
...
int8_t Screen[SCREENW*SCREENH/8]; // buffer stores the next image to be printed on the screen
